...
class MainAdapter(private val itemSelectedListener: RecyclerItemSelectedListener): RecyclerView.Adapter<MainViewHolder>() {


    var dogBreeds = mutableListOf<DogBreeds>()
    var selectedDogBreed = mutableListOf<SelectedBreedItem>()

    fun setDogBreedList(dogBreeds: List<DogBreeds>) {
    ...
    override fun onBindViewHolder(holder: MainViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView.context).load(dogBreed.image.url).fitCenter().centerCrop().into(holder.imageView1)
        holder.itemView.setOnClickListener{
            itemSelectedListener.onRecyclerViewItemSelected(dogBreeds.get(position))
        }
    }
